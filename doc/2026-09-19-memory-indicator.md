...
On Web, the total is labelled **Total memory estimate** and sums the renderer
JavaScript heap, Worker JavaScript heap, and WASM linear-memory capacity. Each
available shared-runtime value is marked as included in the estimate. A failed
or unavailable sample shows **Memory unavailable** and the popup explains the
condition; the next scheduled sample retries automatically.
