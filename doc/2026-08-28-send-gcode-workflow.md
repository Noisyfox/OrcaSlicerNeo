...
- The Device navigation option is presented alongside the Close and Send
  actions and is persisted as the shared `UserPreferences.ui` preference. It
  defaults to enabled, including when migrating an older preference document
  that lacks the field. The printer selector and this option are disabled while
...
- After a successful Send or Send & Print, the modal counts down for five
  seconds before closing. The persisted checkbox changes the status copy and,
  when enabled, closes the modal first and then asks the app-level navigation
  callback to show Device. Manual close, target changes, errors, cancellation,
  and start failures cancel this navigation. Preference reads and writes are
  best effort and do not block sending when host storage is unavailable.
