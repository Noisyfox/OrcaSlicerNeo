...
| Staging entry point | `scripts/stage.mjs` (run as `pnpm stage:assets`) stages every generated renderer asset: handy models first, then profile packages and the threaded/serial WASM variants. `--soft` keeps mock-mode UI development usable when a WASM build is absent. |
