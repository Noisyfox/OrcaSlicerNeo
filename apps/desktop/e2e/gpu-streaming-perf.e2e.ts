...
test('real Electron WebGL2 GPU streaming benchmark emits 250k/1m evidence', async () => {
  ...
  try {
    ...
    for (const count of [250_000, 1_000_000]) {
      const report = await page.evaluate(async (segmentCount) => {
        const benchmark = (window as unknown as {
          __orcaE2e?: { gpuStreamingBenchmark?: (n: number, options?: { frameCount?: number }) => Promise<{
            segmentCount: number; pageCount: number; staticTextureBytes: number; staticUploadCount: number; indexUploadCount: number; staticUploadMs: number | null;
      ...
      expect(report.staticTextureBytes).toBeGreaterThanOrEqual(count * 2 * 4 * 4);
      expect(report.staticUploadCount).toBe(3);
      expect(report.indexUploadCount).toBeGreaterThan(0);
