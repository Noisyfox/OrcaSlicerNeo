#!/usr/bin/env bash
# ================================================================
# build-windows.sh — Windows (Git Bash) local build driver
# ================================================================
# One entry point for the whole local loop on Windows:
...
#
# Git Bash required (MSYS): cygpath, bash build scripts, emsdk_env.sh.
# PowerShell/cmd are NOT supported by the underlying toolchain.
#
# Usage:
#   bash scripts/build-windows.sh <command> [options]
#
# Commands:
...
#                    full 72 MB resources/profiles tree).
...
ensure_emsdk() {
  ...
  local cand
  for cand in "${EMSDK:-}" "$HOME/emsdk" "$HOME/src/emsdk" /c/emsdk /d/emsdk /f/emsdk; do
  ...
  die "Emscripten not found. Install emsdk and 'source <emsdk>/emsdk_env.sh', or set EMSDK."
}

log()  { printf '\033[1;36m[winbuild]\033[0m %s\n' "$*"; }
die()  { printf '\033[1;31m[winbuild] ERROR:\033[0m %s\n' "$*" >&2; exit 1; }
...
case "$CMD" in
  ...
  env)
    if command -v emcmake >/dev/null 2>&1; then
      ...
      echo "emsdk NOT active. In Git Bash, run:"
      for cand in "${EMSDK:-}" "$HOME/emsdk" "$HOME/src/emsdk" /c/emsdk /d/emsdk /f/emsdk; do
        if [[ -n "$cand" && -f "$cand/emsdk_env.sh" ]]; then
  ...
  boost)
    ensure_emsdk
    [[ -d "$WORK/deps/boost-1.84.0" ]] || die "Boost source not staged — run: bash scripts/build-windows.sh deps"
  ...
  build)
    ensure_emsdk
    [[ -d "$BOOST_STAGE" ]] || die "Boost wasm64 archives missing ($BOOST_STAGE) — run: bash scripts/build-windows.sh boost"
  ...
  quick)
    ensure_emsdk
    [[ -d "$BUILD_DIR" ]] || die "No build tree at $BUILD_DIR — run: bash scripts/build-windows.sh build"
  ...
  smoke)
    for f in out/orca_slice.js fixtures/cube.stl fixtures/config.json; do
      [[ -f "$PKG/$f" ]] || die "Missing $PKG/$f — run: bash scripts/build-windows.sh build"
    done
