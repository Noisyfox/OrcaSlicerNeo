...
json run_filament_mutation(const json& request, const char* label, Mutator mutator)
{
    ...
    try {
        ...
        const auto before_history_model = state().history.entries().empty()
            ? Neo::History::Codec::capture_model_state(state().model, state().mesh_capture_cache, state().mutable_object_capture_cache) : state().history.current().model;
        ...
        try {
            ...
            const auto after_history_model = Neo::History::Codec::capture_model_state(state().model, state().mesh_capture_cache, state().mutable_object_capture_cache);
