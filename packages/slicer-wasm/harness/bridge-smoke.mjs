...
// 4ab. Engine-side primitives (Add Primitive): orc_add_shape mirrors
// OrcaSlicer's ObjectList::load_shape_object → create_mesh →
// load_mesh_object path — the mesh is built in the engine (no staging file)
...
// by the instance offset.
{
  const PRIMITIVES = [
    ...
    { type: 'Cylinder', verts: 362, idx: 2160 },
    { type: 'Sphere', verts: 16022, idx: 96120 },
    { type: 'Cone', verts: 183, idx: 1086 },
    { type: 'Disc', verts: 362, idx: 2160 },
    { type: 'Torus', verts: 14400, idx: 86400 },
  ];
  const near = (v, e) => Math.abs(v - e) < 1e-3;
  for (const p of PRIMITIVES) {
    ...
    check(`primitive object and part both named ${p.type}`,
          structure.ok === true && structure.objects?.length === 1
          && sObj?.name === p.type && sObj?.volumes?.length === 1
  ...
  const bad = callJson('orc_add_shape', ['string', 'string'], ['Pyramid', '']);
