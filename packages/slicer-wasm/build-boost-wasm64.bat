...
REM everything self-contained. -m64 must match the libslic3r object build.
...
REM A stale project-config.jam auto-generated by the bash bootstrap.sh embeds a
REM Windows python path with backslashes; jam treats '\' as an escape, so b2's
REM python probe runs a mangled command ('C:Users' is not recognized). b2 never
REM regenerates the file - delete it defensively before every b2 run.
if exist "%BD%\project-config.jam" del "%BD%\project-config.jam"
