...
export function ObjectList({ sceneInteraction }: { sceneInteraction: SceneInteractionController | null }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (disposed || useSettingsStore.getState().modelRevision !== requestedRevision) return;
      if (r.ok && r.objects) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      if (restorePhase !== 'idle') return;
      const nextProjection = projectSelection(structure, sceneInteraction.selectedVolumes().map((v) => v.buffer), highlightLevel);
      setProjection(nextProjection);
      // Restore projections still need to refresh the visible object list;
      // suppress only the history write generated by that projection.
      if (consumeSnapshotSuppression()) return;
  ...
  }, [consumeSnapshotSuppression, platform, restorePhase, sceneInteraction, structure, highlightLevel, setProjection]);
