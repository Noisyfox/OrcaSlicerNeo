  ...
  try {
    const history = await runProjectHistoryMutation<PlateSessionMutationResult>(
      ...
      async (): Promise<PlateSessionMutationResult> => {
        ...
        if (result.configurationStatus?.state === 'ready' && result.configurationStatus.errors.length > 0)
          throw new Error(result.configurationStatus.errors.join('; '));
        if (!result.plateSession) throw new Error('scoped configuration operation produced no mutation');
        return { ...result.plateSession, nativeScopedConfig: result.nativeScopedConfig };
      },
      null,
      {
 ...
 * legacy shared-setting path. Slice actions await this queue before starting,
